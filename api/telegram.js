  ...
  try {
    ...
    if (action === 'status') {
      ...
      if (ok) {
        ...
        return res.json({ connected:true, name:[me.firstName,me.lastName].filter(Boolean).join(' '), username:me.username||'' });
      }
      await client.disconnect();
    ...
    if (action === 'groups') {
      ...
      const groups = dialogs.filter(d=>d.isGroup||d.isChannel).map(d=>({
        id:d.id.toString(), title:d.title||'Unnamed',
    ...
    // ── COLLECT MEMBERS (UPDATED) ─────────────────────────────
    if (action === 'members') {
      const { groupId, days=7, minMessages=1, maxScan=500, scanAll=false } = body;
      if (!groupId) return res.status(400).json({ error: 'Group ID দাও' });
      const client = await getAuthedClient();

      // scanAll=true → no date filter; days=0 → also no date filter
      const useAllTime = scanAll === true || parseInt(days) === 0;
      const cutoff = useAllTime ? 0 : Math.floor((Date.now() - parseInt(days) * 86400000) / 1000);

      // maxScan: 'all' or large number → 10000 max for safety
      const scanLimit = (maxScan === 'all' || parseInt(maxScan) >= 9999)
        ? 10000
        : Math.min(parseInt(maxScan) || 500, 10000);

      const entity = await client.getEntity(groupId);
      const memberMap = {};
      let offsetId = 0, fetched = 0, done = false;

      while (!done && fetched < scanLimit) {
        const msgs = await client.getMessages(entity, { limit: 100, offsetId: offsetId || undefined });
        if (!msgs?.length) break;
        for (const msg of msgs) {
          if (!useAllTime && msg.date < cutoff) { done = true; break; }
          const s = msg.sender;
    ...
    // ── AUTO REPLY: INIT (get latest msg IDs) ─────────────────
    if (action === 'init-auto-reply') {
      const { groupIds } = body;
      if (!groupIds?.length) return res.json({ success: true, lastIds: {} });
      const client = await getAuthedClient();
      const lastIds = {};
      for (const gid of groupIds) {
        try {
          ...
          lastIds[gid] = msgs.length ? msgs[0].id : 0;
        } catch(e) { lastIds[gid] = 0; }
      }
      await client.disconnect();
      return res.json({ success: true, lastIds });
    }

    // ── AUTO REPLY: CHECK & REPLY ─────────────────────────────
    if (action === 'auto-reply-check') {
      const { groupIds, rules, lastIds = {} } = body;
      // rules format: [{ id, matchType: 'keyword'|'link', keyword, reply }]
      if (!groupIds?.length || !rules?.length) {
        return res.json({ success: true, replied: 0, newLastIds: lastIds });
      }
      ...
      for (const gid of groupIds) {
        try {
          ...
          if (msgs.length > 0) {
            // Update latest ID
            newLastIds[gid] = Math.max(...msgs.map(m => m.id));

            // Process each message (skip own messages & media-only)
            for (const msg of msgs) {
              if (msg.out) continue; // skip own messages
              const text = msg.text || msg.message || '';
              if (!text.trim()) continue;

              for (const rule of rules) {
                if (!rule.reply?.trim()) continue;
                let matched = false;

                if (rule.matchType === 'link') {
                  matched = urlRegex.test(text);
                } else if (rule.matchType === 'keyword' && rule.keyword?.trim()) {
                  matched = text.toLowerCase().includes(rule.keyword.toLowerCase().trim());
                }
                ...
                if (matched) {
                  ...
                  break; // only first matching rule per message
                }
              }
            }
          }
        } catch(e) { console.error('auto-reply group error:', gid, e.message); }
      }

      await client.disconnect();
      return res.json({ success: true, replied: totalReplied, newLastIds });
    }
